...
function getTip() {
    var forge_tips = ['If the fodder is the same class as the receiver, the forge exp from level/rank has a bonus of 10%. 20% if the fodder is the same bot.', 'The forge exp from sig level is constant. It does not benefit from the same class/same bot bonus.', 'The carried-over exp from the fodder is constant. It does not benefit from the same class/same bot bonus.', 'The signature programs generate twice the exp on Sharkticons compared to a normal bot of the same star.', 'The exp of a 2/3/4 star signature program is 31/220/1100 for normal bots and 62/440/2200 for Sharkticons', 'The exp/gold ratio of a 4* bot at R5/50 is higher than at R4/40.', 'To forge is to trade huge amounts of gold/ore/sparks for small increments of bot rating.', 'Apart from increasing attack and health, forging gives your bot a higher percentage from class advantage (167% when max forged)', 'It is recommended to max rank/level your bot before forging.', 'The last 25 forged levels are unlocked by forging the same bot of one tier lower, e.g., forging a 4* Tact Optimus Prime into a 5* Tact Optimus Prime.', 'The high the tier of ore-13, the more gold it needs to be used.', 'The higher the star, the less gold a fodder requires (same level/rank).', 'Pay attention when you\'re near level 100! The preview may reach to level 101 or more if you put as many fodders. But the final result will cap at level 100 anyway. All the redundant fodders are lost'];
...
function check() {
    ...
    var bonus = 1.0;
    var strBonus = "";
    if (nFodderName.indexOf("Sharkticon") > 0) {
        isShark = true;
    ...
    if (nFodderName.substring(0, 2) === nReceiverName.substring(0, 2)) {
        bonus = 1.1;
        strBonus = `(Same class bonus: ${bonus})`;
    }
    if (nFodderName === nReceiverName) {
        bonus = 1.2;
        strBonus = `(Same bot bonus: ${bonus})`;
    }
    ...
    $(document).ready(function () {
        $.getJSON("https://kmcbest.github.io/forgeData.json", function (data, textStatus, jqXHR) {
            ...
            if (nFodderRank > nFodderStar + 1) {
                strReport = `Error: a ${nFodderStar} star bot cannot reach rank ${nFodderRank}!`;
            } else if (nFodderLevel > nFodderRank * 10) {
                strReport = `Error: The max level for rank ${nFodderRank} is ${nFodderRank * 10}!`;
            } else if (nFodderStar > nReceiverStar - 1) {
                strReport = `Error: you cannot forge a ${nFodderStar} star bot in to a ${nReceiverStar} star!`;
            } else {
                ...
                strReport = `### Forge number ${window.totalForges} ###<br /><strong>${nFodderStar} star ${nFodderName}</strong> (fodder) becomes ${fodderFinalExp} forge exp ${strBonus}<br /><strong>${nReceiverStar} star ${nReceiverName}</strong> (receiver) has initially ${receiverInitialExp} forge exp</br>Total exp after forging is ${new_exp}<br /><strong>${nReceiverStar} star ${nReceiverName}</strong> becomes <strong>forge level ${result.targetLevel} with ${result.remainingExp} remaining exp</strong><br />Shards received: <strong>${shards}</strong> ${nFodderStar + 1} star ${strShark}.<br />Rank up cost for <strong>${nFodderStar} star ${nFodderName}</strong> is <font color="red">${gold} gold and ${sparks} sparks</font><br />${result.expToMax} exp left to max forge the receiver, that is, <font color="blue">${Math.ceil(result.expToMax / fodderFinalExp)}</font> more current fodders.`
            }
